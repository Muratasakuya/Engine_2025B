...
void Audio::Load(const std::string& filename, AudioType type) {
	...
	if (ext == ".wav" || ext == ".wave") {
		...
		assert(false && "Unsupported audio format. Use WAV or MP3.");
	}
...
void Audio::PlayInternal(const std::string& name, bool loop, float volume) {
	...
	std::lock_guard<std::mutex> lock(mutex_);
	assert(xAudio2_ && masteringVoice_ && "Audio::Init() must be called before Play()");
...
Audio::SoundData Audio::LoadMp3FileWithMediaFoundation(const std::string& filename) {
	...
	assert(mfStarted_ && "MF must be started in Audio::Init()");
