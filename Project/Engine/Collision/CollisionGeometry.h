...
namespace SakuEngine {
	...
	namespace CollisionShape {
		...
		/// <summary>
		/// 中心座標と半径で定義される球形コライダー。json入出力とデフォルト値生成を提供する。
		/// </summary>
		struct Sphere {
		...
		/// <summary>
		/// 中心座標とextentで定義される軸平行境界ボックス。min/max取得とjson入出力を提供する。
		/// </summary>
		struct AABB {
		...
		/// <summary>
		/// 中心、サイズ、回転で定義される有向境界ボックス。回転編集用のオイラー角と判定用クォータニオンを保持する。
		/// </summary>
		struct OBB {
	...
	namespace Collision {
		...
		bool RectToMouse(const Vector2& center, const Vector2& size,
			const Vector2& anchor, InputViewArea viewArea = InputViewArea::Game);
	}
